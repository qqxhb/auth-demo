package com.qqxhb.auth.shiro.config;
...
import com.qqxhb.auth.shiro.service.UserRealm;
import com.qqxhb.auth.shiro.session.CustomWebSessionManager;
...
public class ShiroConfiguration {
	...
	// 配置自定义的Realm
	@Bean
	public AuthShiroRealm getRealm() {
	...
	public ShiroFilterFactoryBean shirFilter(SecurityManager securityManager) {
		...
		// 4.配置过滤器集合
		/**
		 * key ：访问连接 支持通配符的形式 value：过滤器类型 shiro常用过滤器 anno ：匿名访问（表明此链接所有人可以访问） authc
		 * ：认证后访问（表明此链接需登录认证成功之后可以访问）
		 */
